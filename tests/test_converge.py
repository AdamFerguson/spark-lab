...
class TestConverge(unittest.TestCase):
    ...
    def test_spanning_head_launches_with_placement_hosts(self):
        ...
        plan = converge.build_plan(cfg, rendered, {}, None, allow_restart=True, launch_model=True)
    ...
    def test_spanning_worker_skips_launch(self):
        ...
        cfg.model_host_list = lambda alias: ["host-a", "host-b"]
        rendered = {**recipe_bytes(), **LIT}
        plan = converge.build_plan(cfg, rendered, {}, None, allow_restart=True, launch_model=False)
        descs = [d for d, _ in plan.commands]
    ...
    def test_fresh_model_starts_and_records_state(self):
        ...
        self.assertTrue(
            any(
                "mymodel.yaml" in " ".join(map(str, a)) and "--ensure" in " ".join(map(str, a))
                for d, a in plan.commands
            )
        )
        # state now records the running model, so a later content change is caught
        new_model = converge.compute_model_after_apply(None, "mymodel", plan.current_hash, converged_after(plan, False))
    ...
    def test_changed_prometheus_yml_triggers_hot_reload(self):
        ...
        model = {"name": "mymodel", "hash": state.sha256_bytes(old["sparkrun/recipes/mymodel.yaml"])}
    ...
    def _gateway_plan(self, old_litellm_cfg, new_litellm_cfg, cfg=None):
        ...
        old = {
            **recipe_bytes(),
            **LIT,
            "litellm/model_config.yaml": old_litellm_cfg,
            "litellm/config.yaml": b"gateway: 1\n",
        }
        new = {
            **recipe_bytes(),
            **LIT,
            "litellm/model_config.yaml": new_litellm_cfg,
            "litellm/config.yaml": b"gateway: 1\n",
        }
        files = converge.compute_files_after_apply(old)
        model = {"name": "mymodel", "hash": state.sha256_bytes(old["sparkrun/recipes/mymodel.yaml"])}
    ...
    def test_changed_model_config_triggers_litellm_restart(self):
        ...
        self.assertIn("Restart litellm to apply the changed model list (best-effort)", plan.best_effort)
    ...
    def test_extra_models_change_or_removal_triggers_restart(self):
        old = {
            **recipe_bytes(),
            **LIT,
            "litellm/model_config.yaml": b"same\n",
            "litellm/config.yaml": b"gateway: 1\n",
    ...
    def test_recipe_change_stays_pending_without_flag(self):
        ...
        new_model = converge.compute_model_after_apply(
            model, "mymodel", plan.current_hash, converged_after(plan, False)
        )
    ...
    def test_switch_recipe_stops_old_starts_new(self):
        ...
        self.assertTrue(
            any("llama.yaml" in " ".join(map(str, a)) and "--ensure" in " ".join(map(str, a)) for d, a in plan.commands)
        )
        self.assertFalse(plan.model_restart_pending)
        self.assertEqual(
            converge.compute_model_after_apply(model, "llama", plan.current_hash, converged_after(plan, True)),
    ...
    def test_switch_recipe_pending_without_flag(self):
        ...
        self.assertFalse(any("Stop model" in d for d, _ in plan.commands))
        self.assertEqual(
            converge.compute_model_after_apply(model, "llama", plan.current_hash, converged_after(plan, False)), model
        )  # stays pending
    ...
    def test_model_launch_backgrounded_with_probe(self):
        ...
        self.assertLess(
            descs.index("Reconcile LiteLLM + monitoring stack (up + remove orphans)"),
            descs.index("Start/ensure model workload (detached)"),
        )
        self.assertLess(
            descs.index("Start/ensure model workload (detached)"), descs.index("Wait for model to be ready (bounded)")
        )
    ...
    def test_execute_spawns_background_commands(self):
        ...
        self.assertEqual(rt.spawned, [["sparkrun", "run", "r.yaml", "--ensure", "--hosts", "x"]])
...
class TestExpandInstallDir(unittest.TestCase):
    ...
    RECIPE = (
        b'executor_config:\n  volumes:\n  - "{install_dir}/flash-next/ple:/ple"\ncommand: |\n  sglang serve {model}\n'
    )
    ...
    def test_expands_only_in_recipe_files(self):
        ...
        self.assertEqual(converge.expand_install_dir("litellm/model_config.yaml", data, "/h"), data)
        # no placeholder -> byte-identical (golden safety)
        plain = b"- /cache/huggingface:/cache/huggingface\n"
        self.assertEqual(converge.expand_install_dir("sparkrun/recipes/m.yaml", plain, "/h"), plain)
        # no concrete base -> left intact rather than corrupted
        self.assertEqual(converge.expand_install_dir("sparkrun/recipes/m.yaml", data, None), data)
    ...
    def test_write_files_expands_recipe_volumes(self):
        ...
        converge.write_files(types.SimpleNamespace(install_dir=str(d)), rendered, dry_run=False, fs=fs)
        ...
        self.assertEqual((d / "litellm" / "model_config.yaml").read_bytes(), b"model_list: []")
...
class TestTolerantStop(unittest.TestCase):
    ...
    def _run_wrapper(self, fake_body: str) -> subprocess.CompletedProcess:
        ...
        argv = converge.tolerant_stop_argv("sparkrun", "/tmp/x/recipes/r.yaml", ["--hosts", "10.0.0.1"])
        return subprocess.run(argv, env=env, capture_output=True, text=True)

    def test_job_not_found_is_already_converged(self):
        p = self._run_wrapper(
            "echo \"Error: No running workload matches intent 5b98ae96 on hosts ['10.0.0.1']\" >&2\nexit 1"
        )
