...
def make_dir(d: Path, config_text: str = CONFIG_YAML, recipes: dict | None = None) -> Path:
...
class TestResolution(unittest.TestCase):
    ...
    def test_reference_maps_recipe_fields_into_model_block(self):
        ...
        self.assertEqual(m["params"], {"kv_cache_dtype": "fp8_e4m3", "max_running_requests": 4})
    ...
    def test_inline_keys_win_over_recipe(self):
        make_dir(
            ...
            config_text=CONFIG_YAML.replace(
                "    recipe: test-model", "    recipe: test-model\n    port: 40000\n    params:\n      custom_flag: 7"
            ),
        )
        m = load(self.d).model
    ...
    def test_block_without_recipe_passes_through(self):
        text = CONFIG_YAML.replace("    recipe: test-model\n", "").replace(
            "    hf_token_env: HF_TOKEN", "    hf_model: test-llm/alpha\n    runtime: sglang"
        )
...
"""
    ...
    def test_missing_recipe_file_names_available(self):
        make_dir(self.d, config_text=CONFIG_YAML.replace("recipe: test-model", "recipe: nope"))
    ...
    def test_name_stem_mismatch(self):
        make_dir(self.d, recipes={"test-model": RECIPE_YAML.replace("name: test-model", "name: other-name")})
        with self.assertRaises(RecipeError) as cm:
            load(self.d)
        self.assertIn("other-name", str(cm.exception))

    def test_unknown_top_level_key_rejected(self):
        make_dir(self.d, recipes={"test-model": RECIPE_YAML + "sparklab:\n  litellm: {}\n"})
        with self.assertRaises(RecipeError) as cm:
            load(self.d)
    ...
    def test_missing_model_rejected(self):
        make_dir(self.d, recipes={"test-model": RECIPE_YAML.replace("model: test-llm/alpha\n", "")})
        with self.assertRaises(RecipeError):
            load(self.d)
...
class TestLayoutPins(unittest.TestCase):
    ...
    def test_spanning_pins_each_host_in_config_order(self):
        make_dir(
            self.d,
            config_text=CONFIG_YAML.replace(
                "  - name: alpha\n    remote: false", "  - name: 10.0.0.1\n    remote: true"
            )
            .replace("hosts: [alpha]", "hosts: [10.0.0.1, 10.0.0.2]")
            .replace("install_dir: ~/AI", "install_dir: ~/AI\n  hosts: [10.0.0.1, 10.0.0.2]"),
            recipes={"test-model": RECIPE_YAML.replace("min_nodes: 1", "min_nodes: 2")},
        )
        view = load(self.d).view_for("10.0.0.1")
        self.assertEqual(recipe_mod.layout_for_view(view), [("10.0.0.1", [0]), ("10.0.0.2", [1])])
    ...
    def test_spanning_placement_is_run_pool_no_install_hosts(self):
        ...
        make_dir(
            self.d,
            config_text=CONFIG_YAML.replace("hosts: [alpha]", "hosts: [alpha, beta]"),
            recipes={"test-model": RECIPE_YAML.replace("min_nodes: 1", "min_nodes: 2")},
        )
        view = load(self.d).view_for("alpha")
        self.assertEqual(recipe_mod.layout_for_view(view), [("alpha", [0]), ("beta", [1])])
        rendered = render.render(view, self.d / "deploy")
        text = next(v for k, v in rendered.items() if k.startswith("sparkrun/recipes/")).decode()
    ...
    def test_layout_uses_host_ip_when_set(self):
        ...
        make_dir(
            self.d,
            config_text=CONFIG_YAML.replace(
                "  - name: alpha\n    remote: false", "  - name: alpha\n    remote: false\n    ip: 10.0.9.1"
            )
            .replace(
                "  - name: beta\n    ssh: beta.tailx.ts.net\n    remote: true",
                "  - name: beta\n    ssh: beta.tailx.ts.net\n    remote: true\n    ip: 10.0.9.2",
            )
            .replace("hosts: [alpha]", "hosts: [alpha, beta]"),
            recipes={"test-model": RECIPE_YAML.replace("min_nodes: 1", "min_nodes: 2")},
        )
        view = load(self.d).view_for("alpha")
        self.assertEqual(recipe_mod.layout_for_view(view), [("10.0.9.1", [0]), ("10.0.9.2", [1])])
        rendered = render.render(view, self.d / "deploy")
        text = next(v for k, v in rendered.items() if k.startswith("sparkrun/recipes/")).decode()
    ...
    def test_json_string_default_renders_as_yaml_string(self):
        ...
        rec = {
            ...
            "defaults": {
                "host": "0.0.0.0",
                "port": 8888,
                "speculative_config": '{"method":"dflash","model":"incoai/x","num_speculative_tokens":7}',
            },
        }
        text = (
            _yaml.safe_dump(rec, sort_keys=False).rstrip()
            + "\ncommand: |\n  vllm serve {model} '\\n    --speculative-config '\n"
        )
        make_dir(self.d, recipes={"test-model": text})
        rendered = render.render(load(self.d).view_for("alpha"), self.d / "deploy")
        out = next(v for k, v in rendered.items() if k.startswith("sparkrun/recipes/")).decode()
        doc = _yaml.safe_load(out)
        self.assertIsInstance(doc["defaults"]["speculative_config"], str)
        self.assertEqual(_json.loads(doc["defaults"]["speculative_config"])["model"], "incoai/x")

    def test_span_fewer_hosts_than_min_nodes_fails_at_load(self):
        make_dir(
            self.d, config_text=CONFIG_YAML, recipes={"test-model": RECIPE_YAML.replace("min_nodes: 1", "min_nodes: 2")}
        )
    ...
    def test_span_check_skips_inactive_models(self):
        ...
        make_dir(
            self.d,
            config_text=CONFIG_YAML.replace(
                "    recipe: test-model", "    active: false\n    hosts: []\n    recipe: test-model"
            ),
            recipes={"test-model": RECIPE_YAML.replace("min_nodes: 1", "min_nodes: 2")},
        )
...
class TestEnvRendering(unittest.TestCase):
    ...
    def test_model_env_and_injected_token_line_style(self):
        make_dir(self.d, recipes={"test-model": RECIPE_YAML + 'env:\n  PYTHONUNBUFFERED: "1"\n'})
        rendered = render.render(load(self.d).view_for("alpha"), self.d / "deploy")
        text = next(v for k, v in rendered.items() if k.startswith("sparkrun/recipes/")).decode()
        ...
        self.assertLess(env.index("PYTORCH_CUDA_ALLOC_CONF"), env.index("PYTHONUNBUFFERED"))
...
class TestPlacementTable(unittest.TestCase):
    ...
    def test_table_rows_and_gateway_names(self):
        make_dir(self.d, config_text=CONFIG_YAML.replace("hosts: [alpha]", "hosts: [alpha, beta]"))
        rows = load(self.d).placement_table()
        self.assertEqual(
            rows, [("alpha", "my-model", "test-gateway-name", True), ("beta", "my-model", "test-gateway-name", True)]
        )

    def test_table_shows_unserved_host(self):
        rows = load(make_dir(Path(tempfile.mkdtemp()))).placement_table()
        self.assertEqual(rows, [("alpha", "my-model", "test-gateway-name", True), ("beta", "", "", True)])
...
class TestRepoRecipeCompliance(unittest.TestCase):
    ...
    def test_all_repo_recipes_are_sparkrun_native(self):
        ...
        for path in files:
            ...
            self.assertEqual(str(data.get("name") or ""), path.stem, f"{path.name}: inner name != file stem")
            self.assertTrue(str(data.get("model") or "").strip(), f"{path.name}: missing model:")
