...
| 2 | **Full suite green in CI:** the entire test suite passes on the branch in CI | `.github/workflows/validate.yml` (unit tests + coverage ≥ 85%) | ✅ PASS (81 tests) |
| 3 | **Coverage** on `sparklab/` ≥ 85% (CI-gated) | `coverage report` | ✅ PASS (87%) |
...
| 7 | **Staging E2E:** full converge/restart/discovery/upgrade flow passes on a staging node | `docs/STAGING_E2E.md`, `scripts/staging.sh` | ⏳ **PENDING — staging Spark** |
| 8 | **Rollback drills:** reverting a config change converges the node back | `scripts/staging.sh rollback` | ⏳ **PENDING — staging Spark** |
| 9 | **No live-node disturbance:** all E2E ran on a staging node + distinct install_dir; live model untouched | `scripts/staging.sh` safety guard (`LIVE_INSTALL_DIR`) | ⏳ **PENDING — verified on the staging run** |
