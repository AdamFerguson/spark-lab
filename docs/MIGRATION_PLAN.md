...
- **Phase 3 (next):** CLI hardening + shell→Python parity — port `capture`/`secret-scan` to Python, console entry point, UX (`logs`/`validate`/`--dry-run` breadth/progress), gated by the Phase-2 parity tests.
