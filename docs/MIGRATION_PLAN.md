...
- **Phase 5 (next):** recipe system -- standalone in-repo `recipes/*.yaml` (the sparkrun registry, `docs/REGISTRY.md`) + discovery adapters (ADR 0003: sparkrun registries, SGLang cookbook) with `recipes search/list/show` + opt-in LLM-assisted conversion (candidate, never auto-applied).
