...
- **Phase 6 (next):** integration validation + merge gate -- full E2E on the **staging** node (multi-model + recipes + monitoring), executed rollback drills, final security/supply-chain + secrets audit (incl. the LLM call path: token via env only, minimal content, no secrets in prompts), full regression sweep (parity + entire suite green), and the documented main-branch merge gate. Legacy shell path stays operational until merge + a deprecation window.
