...
In remote operator mode the same read-only adoption runs against the remote
node, and the state file is written *on the managed node* (in its spark-lab
checkout) so local and remote operation share one source of truth.
"""
...
from ..core import config, node, render, state
...
def run(args) -> int:
    ...
    install_desc = cfg.install_dir_raw if cfg.is_remote else str(cfg.install_dir)
    ...
    if recipe_bytes is not None:
        ...
        sys.stderr.write(
            f"\nNOTE: active recipe '{cfg.recipe_name}' not found in {install_desc}/sparkrun/recipes. "
    ...
    print(f"   install dir : {install_desc}" + (" (remote)" if cfg.is_remote else ""))
