...
def entry_for(served_model: str, address: str, port: int,
              public_name: str | None = None) -> dict:
...
def add_extra_model(config_path: Path, entry: dict) -> None:
    ...
    if any(m.get("model_name") == entry["model_name"] for m in models):
        raise ValueError(f"extra_models entry '{entry['model_name']}' already exists "
                         "(edit the config or pick another --public-name)")
    models.append(entry)
...
def run(args) -> int:
    ...
    try:
        ...
        print(f"[ERROR] engine probe failed at http://{address}:{port}/v1/models: {e}\n"
              f"        is the engine up and reachable from here?", file=sys.stderr)
        return 1
    served_model = args.served_model or served[0]
    if served_model not in served:
        print(f"[ERROR] engine serves {served}; '{served_model}' not among it "
              f"(--served-model must match one)", file=sys.stderr)
        return 1
    ...
    if not getattr(args, "dry_run", False):
        try:
            ...
            print(f"[ERROR] {e}", file=sys.stderr)
            return 1
    print(f"{'[dry-run] would add' if getattr(args, 'dry_run', False) else 'Added'} "
    ...
    ns = SimpleNamespace(config=args.config, hosts=None, dry_run=False, no_model=True,
                         restart_model=False, diff=False, verbose=getattr(args, "verbose", False),
                         json=False, runtime=args.runtime)
